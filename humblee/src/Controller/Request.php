...
class Request extends Xhr {
    ...
    public function loadContentMenu(): void
	{
		...
		$li_format = function($item, $slug, $class) { return '<a href="'._app_path.'admin/edit/?page_id='.$item->thisID.'">'.$item->label.'</a>'; };
	...
	public function loadPagesTable(): void
    {
		...
		$li_format = function($item, $slug, $class) { return '<div class="pages_menu_item" data="'.$item->thisID.'"><a '.$class.' title="'.$slug.'">'.$item->label.'</a></div>'; };
		echo $pages->drawMenu_UL($all_pages,array('li_format'=>$li_format,'id_label'=>'pageID_'));
	}
